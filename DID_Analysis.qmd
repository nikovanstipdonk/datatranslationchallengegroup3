...
```{r}
did <- feols(employment_standard ~ covid, data = data_retail)
...
```{r}
did1 <- feols(employment_standard ~ indname + covid + indname * covid, data = data)
...
did2 <- feols(employment_standard ~ covid, data = data_info)
etable(did2)
```
